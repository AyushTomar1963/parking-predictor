...
# MERGED: Added capacity field to sync with frontend
class BookingRequest(BaseModel):
    lot_id: str
    predicted_occupancy: float
    hour_of_day: Optional[int] = None  # if not provided, will use timestamp -> hour
    capacity: Optional[int] = None     # ADDED: prioritize frontend capacity input for testing/simulation
...
@app.post("/api/booking")
async def booking(payload: BookingRequest):
    """
    Compute booking probability given predicted occupancy and a lot id.
    This endpoint will:
      1) load processed lot data (for estimating lambda/mu) -- uses queue_estimator.get_queueing_inputs
      2) call get_booking_confirmation(predicted_occupancy, capacity, arrival_lambda, service_rate_mu)
    """
    # Basic checks
    if get_queueing_inputs is None or get_booking_confirmation is None:
        raise HTTPException(status_code=500, detail="Queueing functions not implemented or import failed.")
    ...
    # MERGED: Prioritize frontend-provided capacity for testing/simulation; fall back to data
    ...
    # Compute hour — default to 12 if not provided
    hour = payload.hour_of_day if payload.hour_of_day is not None else 12
    ...
    # Resolve arrival rate for the requested hour
    arrival_rate = hourly_map.get(hour, hourly_map.get(str(hour), 1.0)) if hourly_map else 1.0

    # Call booking confirmation with paper-faithful signature
    try:
        logger.info(f"Calling get_booking_confirmation with: occ={payload.predicted_occupancy}, cap={capacity}, arrival_lambda={arrival_rate}, mu={service_rate_mu}")
        confirmation = get_booking_confirmation(
            predicted_occupancy=payload.predicted_occupancy,
            capacity=capacity,
            arrival_lambda=arrival_rate,
            service_rate_mu=service_rate_mu
        )
        logger.info(f"Booking result: {confirmation}")
        return confirmation
    except Exception as e:
        logger.error(f"Booking confirmation calculation failed: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Queueing calculation error: {str(e)}")
